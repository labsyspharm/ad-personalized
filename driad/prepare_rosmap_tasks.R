...
rosmap_quant_clinical_split <- rosmap_quant_clinical %>%
  ...
  mutate(path = paste0("driad/rosmap_quant_clinical_", brain_region, "_", classification, "_", class, ".tsv.gz"))

pwalk(
  rosmap_quant_clinical_split,
  function(path, data, ...)
...
prediction_tasks <- rosmap_quant_clinical_split %>%
  ...
  filter(
    brain_region == "PCC",
    classification == "high_threshold"
  ) %>%
  rowwise() %>%
  mutate(
    task = DRIAD::prepareTask(path, comparison) %>%
      list()
  ) %>%
  select(-path) %>%
  ungroup() %>%
...
qsave(
  prediction_tasks_pairs, "driad/prediction_tasks.qs"
)
...
set.seed(42)
background_gene_sets <- prediction_tasks_pairs %>%
  select(-task, -pairs) %>%
  crossing(
  ...
  rowwise() %>%
  mutate(
    genes = map2(gene_set_size, 1:1000, ~sample(gene_universe, .x, replace = FALSE)) %>% {
      tibble(seq_id = 1:1000, genes = .)
    } %>%
      list()
  ) %>%
  ungroup() %>%
  unnest(genes) %>%
  mutate(
    gs_id = paste0("background_", gene_set_size, "_", seq_id, "_", id)
  )
