...
```{r}
compound_and_background_res <- syn("syn64765893") %>%
  read_csv()

compound_res_sig <- syn("syn64768590") %>%
  qread()
```
...
closest_gene_set_size <- crossing(
  gene_set_size_compound = compound_and_background_res %>%
    filter(
      gs_type == "compound"
    ) %>%
    pull(gene_set_size) %>%
    unique(),
  gene_set_size_background = compound_and_background_res %>%
    filter(
      gs_type == "background"
    ) %>%
    pull(gene_set_size) %>%
  ...
  mutate(
    diff = abs(gene_set_size_compound - gene_set_size_background)
  ) %>%
  arrange(diff) %>%
  group_by(gene_set_size_compound) %>%
  slice(1) %>%
  ungroup()


compound_and_background_res_name <- compound_and_background_res %>%
  power_left_join(
    compound_res_sig %>%
      select(name = drug, experiment, gs_id),
    by = "gs_id",
    check = check_specs(
...
compound_and_background_res_long <- bind_rows(
  compound_and_background_res_name %>%
    filter(gs_type == "compound"),
  compound_and_background_res_name %>%
    filter(gs_type == "compound") %>%
    select(name, experiment, gene_set_size_compound = gene_set_size) %>%
    power_inner_join(
      closest_gene_set_size,
      by = "gene_set_size_compound",
      check = check_specs(
    ...
    power_inner_join(
      ...
      by = c("gene_set_size_background" = "gene_set_size"),
      check = check_specs(
        unmatched_keys_left = "warn"
      )
    )
...
compound_res_p_values <- compound_and_background_res %>%
  ...
  power_inner_join(
    closest_gene_set_size,
    by = c("gene_set_size" = "gene_set_size_compound"),
    check = check_specs(
      unmatched_keys_left = "warn",
      unmatched_keys_right = "warn",
      duplicate_keys_right = "warn"
  ...
  nest_join(
    compound_and_background_res %>%
      filter(gs_type == "background"),
    by = join_by("brain_region", "classification", "class", "comparison", "gene_set_size_background" == "gene_set_size"),
    name = "background_res"
  ) %>%
  mutate(
...
```{r}
plot_dir <- file.path("driad_plots", "additional_jaks")
    ...
    plot = map2(
      ...
      function(d, pvals) {
        d %>%
          ...
          facet_wrap(
            vars(interaction(name, experiment), class)
          ) +
          theme_bw() +
          theme(
